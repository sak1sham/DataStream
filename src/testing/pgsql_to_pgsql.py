...
class SqlTester():
    ...
    def check_match(self, record, pgsql_record, column_dtypes) -> bool:
            ...
            try:
                ...
                if record[key] and not key.startswith('parquet_format'):
                    if column_dtypes[key].startswith('timestamp') or column_dtypes[key].startswith('date'):
                        if record[key] is not pd.NaT and pgsql_record[pgsql_key] is not pd.NaT:
                            pgsql_record[pgsql_key] = int((pytz.utc.localize(pgsql_record[pgsql_key])).timestamp())
                            record[key] = int((record[key]).timestamp())
                        else:
                            pgsql_record[pgsql_key] = None
                            record[key] = None
                        ...
                        if np.isnan(record[key]) or not record[key]:
                            ...
                            pgsql_record[pgsql_key] = int(pgsql_record[pgsql_key])
                            record[key] = int(record[key])

                    assert record[key] == pgsql_record[pgsql_key]
            except Exception as e:
                logger.err(str(key))
                logger.err(str(record[self.primary_key]))
                logger.err("Source: " + str(record[key]))
                logger.err("Destination: " + str(pgsql_record[pgsql_key]))
                raise
        return True
                ...
                for _ in range(self.N):
                    ...
                    if(data_df.shape[0]):
                        if('bookmark' in self.table_map.keys() and self.table_map['bookmark']):
                            ...
                            print("After filter:", data_df[self.table_map['bookmark']].max())
                        str_id = ""
                        ...
                        conn = psycopg2.connect(
                            host = self.db['destination']['url'],
                            database = self.db['destination']['db_name'],
                            user = self.db['destination']['username'],
                        ...
                        for _, row in data_df.iterrows():
                            ...
                            try:
                                assert self.check_match(row, pgsql_record[0], column_dtypes)
                            except Exception as e:
