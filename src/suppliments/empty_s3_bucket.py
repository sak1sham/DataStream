    ...
    if(len(args) > 0):
        try:
            ...
            for obj in objects_for_deletion:
                ...
                count += 1
            logger.inform(s = "Deleted {0} objects from bucket {1}.".format(count, args[0]))
            if('notify' in settings.keys() and settings['notify']):
                send_message(msg = "Deleted {0} objects from S3 bucket {1}.".format(count, args[0]), channel=settings['slack_notif']['channel'], slack_token=settings['slack_notif']['slack_token'])
